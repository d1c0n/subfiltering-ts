...
export class TwigToPh extends AbstractHandler {
  /**
   * TestSet:
   * ```
  ...
  public transform(segment: string): string {
    ...
    while ((match = regex.exec(segment)) !== null) {
      ...
      if (!twigVariable.includes(ConstantEnum.GTPLACEHOLDER)) {
        ...
        const replacement = `<ph id="${this.pipeline.getNextId()}" ctype="${CTypeEnum.TWIG}" equiv-text="base64:${Buffer.from(twigVariable).toString('base64')}"/>`;
