...
export class DoubleSquareBrackets extends AbstractHandler {
  /**
   * @inheritDoc
   */
  public transform(segment: string): string {
    ...
    while ((match = regex.exec(segment)) !== null) {
      ...
      if (!snailVariable.includes(ConstantEnum.GTPLACEHOLDER)) {
        ...
        const replacement = `<ph id="${this.pipeline.getNextId()}" ctype="${CTypeEnum.DOUBLE_SQUARE_BRACKETS}" equiv-text="base64:${Buffer.from(snailVariable).toString('base64')}"/>`;
