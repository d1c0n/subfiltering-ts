...
describe('MyMemorySubFilteringTest', () => {
  ...
  it('testSingleCurlyBrackets', () => {
    ...
    assert.strictEqual(segmentL1, string_from_UI);
    assert.strictEqual(segment, filter.fromLayer1ToLayer0(segmentL1));
  });

  it('testVariablesWithHTML', () => {
    const filter = getFilterInstance();
    ...
    assert.strictEqual(segment_from_UI, filter.fromLayer0ToLayer1(db_segment, 'airbnb'));
  });
  ...
  it('testDecodeInternalEncodedXliffTags', () => {
    const filter = getFilterInstance();
    ...
    assert.strictEqual(db_segment, filter.fromLayer1ToLayer0(segment_received));
    assert.strictEqual(segment_received, filter.fromLayer0ToLayer1(db_segment));
  });
  ...
  it('testWithSquareSprintf', () => {
    ...
    const tags = [
      ...
      '[%.2f:placeholder]', '[%1$s:placeholder]', '[%1$i:placeholder]', '[%f:placeholder]',
      '[%1$.2f:placeholder]',
    ];
